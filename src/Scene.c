...
void Scene_Update()
{
    ...
    for (unsigned int x = 0; x < x_size; x++)
    {
        for (unsigned int y = 0; y < y_size; y++)
        {
            ...
            Ray_t cameraRay = Camera_Generate_Ray(&Scene.cam, normX, normY);
            ...
            if (intersection_found)
            {
                ...
                if (tmp)
                {
                    ...
                    const vec3 colour = Material_Compute_Colour(Scene.objects, Scene.lights, closest_object_index, &closestIntPoint, &closestLocalNormal, &cameraRay);
