...
inline Scene Scene_Multiple_Shapes(void)
{
    Scene scene;
    ...
    scene.objects.shapes[0] = (Shape){.type      = SHAPE_CONE,
                                      ...
                                      .transform = Transform_Set((vec3){0.0, 0.0, -0.5}, (vec3){0.0, 0.0, 0.0}, (vec3){1.0, 1.0, 2.0}, false)};

    scene.objects.shapes[1] = (Shape){.type      = SHAPE_SPHERE,   // Left Sphere
                                      .mat       = &scene.mats[2], // Blue Diffuse
                                      .transform = Transform_Set((vec3){1.5, -2.0, 0.5}, (vec3){0.0, 0.0, 0.0}, (vec3){0.5, 0.5, 0.5}, false)};

    scene.objects.shapes[2] = (Shape){.type      = SHAPE_SPHERE,   // Right Sphere
                                      .mat       = &scene.mats[3], // Yellow Diffuse
                                      .transform = Transform_Set((vec3){1.5, 0.0, 0.0}, (vec3){0.0, 0.0, 0.0}, (vec3){1.0, 1.0, 1.0}, false)};

    scene.objects.shapes[3] = (Shape){.type      = SHAPE_SPHERE,   // Top Sphere
                                      .mat       = &scene.mats[4], // Orange Diffuse
                                      .transform = Transform_Set((vec3){0.0, 0.0, -1.0}, (vec3){0.0, 0.0, 0.0}, (vec3){0.5, 0.5, 0.5}, false)};
    ...
    scene.objects.shapes[7] = (Shape){.type      = SHAPE_CYLINDER, // Cylinder1
                                      ...
                                      .transform = Transform_Set((vec3){-1.5, -2.0, 1.0}, (vec3){0.0, -M_PI / 2.0, 0.0}, (vec3){0.25, 0.25, 1.0}, false)};

    scene.objects.shapes[8] = (Shape){.type      = SHAPE_CYLINDER, // Cylinder2
                                      .mat       = &scene.mats[0], // Silver Metal
                                      .transform = Transform_Set((vec3){-1.0, -2.0, 0.0}, (vec3){0.0, 0.0, 0.0}, (vec3){0.25, 0.25, 1.0}, false)};

    scene.objects.shapes[9] = (Shape){.type      = SHAPE_CONE,     // Cone2
                                      .mat       = &scene.mats[1], // Gold Metal
                                      .transform = Transform_Set((vec3){0.0, -1.0, 0.0}, (vec3){M_PI / 4.0, 0.0, 0.0}, (vec3){0.5, 0.5, 1.0}, false)};
